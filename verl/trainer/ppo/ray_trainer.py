...
from router_r1.llm_agent.generation import LLMGenerationManager, GenerationConfig
...
def compute_data_metrics(batch, use_critic=True):
    ...
    sequence_metric = batch.batch['token_level_metric'].sum(-1)
    sequence_cost = batch.batch['token_level_cost'].sum(-1)
    ...
    metrics = {
        # cost
        'critic/cost/mean':
            torch.mean(sequence_cost).detach().item(),
        'critic/cost/max':
            torch.max(sequence_cost).detach().item(),
        'critic/cost/min':
            torch.min(sequence_cost).detach().item(),
        # metric
        'critic/metric/mean':
            torch.mean(sequence_metric).detach().item(),
        'critic/metric/max':
            torch.max(sequence_metric).detach().item(),
        'critic/metric/min':
            torch.min(sequence_metric).detach().item(),
        # score
        'critic/score/mean':
    ...
    if 'valid_route_stats' in batch.meta_info:
        metrics['env/number_of_valid_route'] = float(np.array(batch.meta_info['valid_route_stats'], dtype=np.int16).mean())
...
class RayPPOTrainer(object):
    ...
    def _validate(self):
        ...
        import torch
        reward_tensor_lst = []
        metric_em_tensor_lst = []
        metric_f1_tensor_lst = []
        cost_tensor_lst = []
        route_tensor_lst = []
        data_source_lst = []
        ...
        if not self.config.do_route:
            for test_data in self.val_dataloader:
                ...
                metric_f1_tensor_lst.append(metric_f1_tensor)
                metric_em_tensor_lst.append(metric_em_tensor)
        ...
        cost_tensor = torch.cat([rw.sum(-1) for rw in cost_tensor_lst], dim=0).cpu()  # (batch_size,)
        metric_f1_tensor = torch.cat([rw.sum(-1) for rw in metric_f1_tensor_lst], dim=0).cpu()  # (batch_size,)
        metric_em_tensor = torch.cat([rw.sum(-1) for rw in metric_em_tensor_lst], dim=0).cpu()  # (batch_size,)
        route_tensor = torch.cat([rw.sum(-1) for rw in route_tensor_lst], dim=0).cpu()  # (batch_size,)
        # reward_tensor = torch.cat(reward_tensor_lst, dim=0).sum(-1).cpu()  # (batch_size,)
        data_sources = np.concatenate(data_source_lst, axis=0)
        # evaluate test_score based on data source
        data_source_reward = {}
        data_source_cost = {}
        data_source_metric_f1 = {}
        data_source_metric_em = {}
        data_source_route = {}
        for i in range(reward_tensor.shape[0]):
            data_source = data_sources[i]
            if data_source not in data_source_reward:
                data_source_reward[data_source] = []
                data_source_cost[data_source] = []
                data_source_metric_f1[data_source] = []
                data_source_metric_em[data_source] = []
                data_source_route[data_source] = []
            data_source_reward[data_source].append(reward_tensor[i].item())
            data_source_cost[data_source].append(cost_tensor[i].item())
            data_source_metric_f1[data_source].append(metric_f1_tensor[i].item())
            data_source_metric_em[data_source].append(metric_em_tensor[i].item())
            data_source_route[data_source].append(route_tensor[i].item())


        metric_dict = {}
        for data_source, rewards in data_source_reward.items():
            metric_dict[f'val/test_score/reward/{data_source}'] = np.mean(rewards)
        for data_source, costs in data_source_cost.items():
            metric_dict[f'val/test_score/cost/{data_source}'] = np.mean(costs)
        for data_source, metric_f1 in data_source_metric_f1.items():
            metric_dict[f'val/test_score/F1/{data_source}'] = np.mean(metric_f1)
        for data_source, metric_em in data_source_metric_em.items():
            metric_dict[f'val/test_score/EM/{data_source}'] = np.mean(metric_em)
        for data_source, route_cnt in data_source_route.items():
            metric_dict[f'val/test_score/route_cnt/{data_source}'] = np.mean(route_cnt)
    ...
    def _balance_batch(self, batch: DataProto, metrics, logging_prefix='global_seqlen'):
        ...
        batch.meta_info['batch_completion_tokens'] = np.array(batch.meta_info['batch_completion_tokens'])[global_idx].tolist()
        # print(global_idx, len(global_idx))
            ...
            for batch_idx, batch_dict in enumerate(self.train_dataloader):
                ...
                # with _timer('step', timing_raw):
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        metric_tensor, cost_tensor, reward_tensor = self.reward_fn(batch)
                        batch.batch['token_level_scores'] = reward_tensor
                        batch.batch['token_level_metric'] = metric_tensor
                        batch.batch['token_level_cost'] = cost_tensor
                    ...
                    if self.config.trainer.critic_warmup <= self.global_steps:
                        ...
                        with _timer('update_actor', timing_raw):
                            if self.config.do_route and self.config.actor_rollout_ref.actor.state_masking:
