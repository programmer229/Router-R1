...
find_latest_checkpoint() {
    ...
    local latest
    latest=$(ls -d "$ckpt_dir"/global_step_* 2>/dev/null | sort -t_ -k3,3n | tail -n1)
    if [ -z "$latest" ]; then
        return
    fi

    if compgen -G "$latest/pytorch_model*.bin" > /dev/null || \
       compgen -G "$latest"/*.safetensors > /dev/null || \
       [ -f "$latest/pytorch_model.bin.index.json" ] || \
       [ -f "$latest/model.safetensors.index.json" ]; then
        printf '%s' "$latest"
    fi
}
...
if [ -n "$LOAD_ACTOR_CKPT" ]; then
    ...
    CMD_ARGS+=("+actor_rollout_ref.resume_from_checkpoint=$LOAD_ACTOR_CKPT")
fi

if [ -n "$LOAD_CRITIC_CKPT" ]; then
    echo "Auto-loading critic checkpoint from $LOAD_CRITIC_CKPT"
    CMD_ARGS+=("+critic.resume_from_checkpoint=$LOAD_CRITIC_CKPT")
fi

PYTHONUNBUFFERED=1 NCCL_P2P_DISABLE=1 NCCL_IB_DISABLE=1 python3 -m verl.trainer.main_ppo "${CMD_ARGS[@]}" \
    2>&1 | tee "$EXPERIMENT_NAME.log"
